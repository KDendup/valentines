...
export const InstancedHeart = ({ appState, colorTheme, gestureData }: any) => {
  ...
  const data = useMemo(() => {
    ...
    // 心形生成函数
    const getHeartPosition = (t: number, variation: number) => {
      // 2D心形参数方程
      ...
      // 添加随机偏差使其更自然
      const scale = 0.65 * (0.8 + variation * 0.2);
    ...
    for (let i = 0; i < LEAF_COUNT; i++) {
      ...
      const heartPos = getHeartPosition(t, variation);
  ...
  useFrame((state, delta) => {
    ...
    if (orbitRef.current) {
        ...
        for (let i = 0; i < ORBIT_PARTICLE_COUNT; i++) {
            ...
            const target = isTree ? heartPos : explodeTarget;
