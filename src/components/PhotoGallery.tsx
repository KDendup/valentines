...
const heartCenter = new THREE.Vector3(0, 1, 0);
...
export const PhotoGallery = ({ photos, appState }: PhotoGalleryProps) => {
  ...
  const daysCount = useMemo(() => {
    const start = new Date('2024-09-27').getTime();
  ...
  const layout = useMemo(() => {
    ...
    return photos.map((_, i) => {
      ...
      const viewScale = 0.85; 
      const zLayer = (i % 3) * 0.5;
      
      const viewPos = new THREE.Vector3(
        xBase * rSpread * viewScale, 
        (yBase * rSpread * viewScale) + 3.5, 
        14 - zLayer 
      );

      return { heartPos, viewPos };
    });
  }, [photos]);
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      groupRef.current.children.forEach((child, i) => {
        ...
        const { heartPos, viewPos } = layout[i];
        
        let target = isExplode ? viewPos : heartPos;
        ...
        if (isExplode || isFocused) {
          ...
          // 情况B: 在心形上时，执行动态混合旋转算法
          
          // 目标1 (Radial): 之前的效果，水平背对心形向外看
          // 计算方法：取当前位置，Y轴设为心形中心高度，得到的向量就是水平向外的方向
          tempRadialVec.copy(child.position).setY(heartCenter.y);
          // 让目标点远离心形中心，形成朝向
